...
class TetrisGame:
    ...
    def draw_grid(self):
        """Draw the game grid"""
        # Draw background
        grid_rect = pygame.Rect(GRID_X, GRID_Y, GRID_WIDTH * BLOCK_SIZE, GRID_HEIGHT * BLOCK_SIZE)
        ...
        if self.clearing_lines:
            ...
            for y in self.clearing_lines:
                for x in range(GRID_WIDTH):
                    rect = pygame.Rect(GRID_X + x * BLOCK_SIZE + 1,
        ...
        if self.current_piece and self.show_ghost and not self.clearing_lines:
            ...
            for x, y in ghost.get_blocks():
                if y >= 0:
                    rect = pygame.Rect(GRID_X + x * BLOCK_SIZE + 2,
        ...
        if piece:
            ...
            for row_idx, row in enumerate(piece.shape):
                for col_idx, cell in enumerate(row):
                    if cell:
                        rect = pygame.Rect(offset_x + col_idx * BLOCK_SIZE,
